...
$decode  = array( "'", "\"" ,">", "<" , "&");
$xml_docs = "<lat>". $edit->lat. "</lat><long>". $edit->long. "</long><location>". $edit->location ."</location><name>". $edit->name. "</name><number>". $edit->number. "</number><time>". $edit->time. "</time><user>". $edit->user. "</user>";
...
$refreshed_contents4 = str_replace($decode, $secret_code, $edit->number);
$refreshed_contents5 = str_replace($decode, $secret_code, $edit->time);
$refreshed_contents6 = str_replace($decode, $secret_code, $edit->user);
$refreshed_contents7 = str_replace($decode, $secret_code, $edit->location);
$xml_docs = "<lat>". $refreshed_contents1. "</lat><long>". $refreshed_contents2. "</long><location>". $refreshed_contents7 ."</location><name>". $refreshed_contents3. "</name><number>". $refreshed_contents4. "</number><time>". $refreshed_contents5. "</time><user>". $refreshed_contents6. "</user>";
$new_contents[] = $xml_docs;
}
//##########Create the file to be saved ##########
$XML = "<?xml version='1.0'?><data>";
foreach($new_contents as $edited_data){
$XML .= "<item>". $edited_data ."</item>";}
$XML .= "</data>";
